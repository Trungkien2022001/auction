...
async function getRandomActiveAuction(userId) {
    const auctions = await knex("auction").select().where("status", "2").andWhere("seller_id", "<>", userId)

    return getSample(auctions, _.random(30, auctions.length < 60 ? auctions.length : 60))
}
...
async function runMockRaise() {
    ...
    for (let i = 0; i < auctions.length; i++) {
        ...
        logger.info(`Fake raise success, auction_id: ${auction.id}, user_id: ${user.id}, raise_price: ${priceRaise}`)
    }
